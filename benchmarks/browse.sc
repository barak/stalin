...
;;; A simple implementation of GENSYM, Qobi
...
;;; begin Stalin
 (define (gensym) (string->uninterned-symbol (string-copy "gensym")))
;;; end Stalin
	...
	(else (cond ((not (pair? (car pat)))
		     (cond ((eq? (string-ref (symbol->string (car pat)) 0) #\?)
			    (let ((val (assv (car pat) alist)))
			     (cond (val (match (cons (cdr val) (cdr pat))
					       dat alist))
				   (else (match (cdr pat)
						(cdr dat)
						(cons (cons (car pat)
			   ...
			   ((eq? (string-ref (symbol->string (car pat)) 0) #\*)
			    (let ((val (assv (car pat) alist)))
			     (cond (val (match (append (cdr val) (cdr pat))
					       dat alist))
				   (else
				    (do ((l '()
					    (append! l
