	    ...
	    (cond
	     ...
	     ((pair? es)
	      (pe* (cons (dynamic-parse-expression env (car es)) results)
		   (cdr es)))
	     (else (error 'dynamic-parse-expression*
			  "Not a list of expressions: ~s" es))))))
  (reverse (pe* '() exprs))))
...
(define (dynamic-parse-case-clause env e)
 (if (pair? e)
	...
	(map dynamic-parse-datum (car e)))
       (else
	(error 'dynamic-parse-case-clause "Not a datum list: ~s" (car e))))
      (dynamic-parse-body env (cdr e)))
...
(define (dynamic-parse-recursive-bindings env bindings)
 ...
 (if (list-of-list-of-2s? bindings)
     ...
     (error
      'dynamic-parse-recursive-bindings "Illegal bindings: ~s" bindings)))
...
(define (dynamic-parse-command* env commands)
 ...
 (if (list? commands)
     ...
     (error
      'dynamic-parse-command* "Invalid sequence of commands: ~s" commands)))
...
(define (dynamic-parse-define env args)
 ...
 (if (pair? args)
	...
	(if (list-of-1? exp-or-body)
	    ...
	    (error
	     'dynamic-parse-define "Not a single expression: ~s" exp-or-body)))
       ((pair? pattern)
...
(define (ast-gen syntax-op arg)
 ...
 (let ((ntvar
	(case syntax-op
	 ...
	 ((28) (gen-tvar))
	 (else
	  (error 'ast-gen "Can't handle syntax operator: ~s" syntax-op)))))
