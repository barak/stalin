...
(define (dynamic-parse-datum e)
 ...
 (cond
  ...
  ((vector? e)
   (dynamic-parse-action-vector-const
    (map dynamic-parse-datum (vector->list e))))
  ((pair? e)
   (dynamic-parse-action-pair-const (dynamic-parse-datum (car e))
				    (dynamic-parse-datum (cdr e))))
  (else (panic "Unknown datum: ~s"))))

;;; VarDef

;;; dynamic-parse-formal: parses nonterminal <variable> in defining occurrence
;;; position
 ...
 (if (symbol? e)
     (cond
      ((memq e syntactic-keywords)
       (panic "Illegal identifier (keyword): ~s"))
      ((dynamic-lookup e f-env)
       (panic "Duplicate variable definition: ~s"))
      (else
 ...
 (letrec ((pe*
	   (lambda (results es)
	    (cond
	     ((null? es) results)
	     ((pair? es)
	      (pe* (cons (dynamic-parse-expression env (car es)) results)
		   (cdr es)))
	     (else (panic "Not a list of expressions: ~s"))))))
  (reverse (pe* '() exprs))))
...
(define (dynamic-parse-expressions env exprs)
 ...
 (cond
  ...
  (else (panic "Illegal expression list: ~s"))))
...
(define (dynamic-parse-quote env args)
 (if (list-of-1? args)
     ...
     (panic "Not a datum (multiple arguments): ~s")))
...
(define (dynamic-parse-if env args)
 (cond
  ...
  (else (panic "Not an if-expression: ~s"))))
...
(define (dynamic-parse-set env args)
 (if (list-of-2? args)
     (dynamic-parse-action-assignment
      ...
      (dynamic-parse-expression env (cadr args)))
     (panic "Not a variable/expression pair: ~s")))
 ...
 (if (and (pair? args) (list? args))
     (dynamic-parse-action-cond-expression
      (map (lambda (e)
	   ...
	   args))
     (panic "Not a list of cond-clauses: ~s")))
...
(define (dynamic-parse-and env args)
 (if (list? args)
     ...
     (panic "Not a list of arguments: ~s")))
...
(define (dynamic-parse-case-clause env e)
 (if (pair? e)
	...
	(map dynamic-parse-datum (car e)))
       (else
	(panic "Not a datum list: ~s")))
      (dynamic-parse-body env (cdr e)))
     (panic "Not case clause: ~s")))
     ...
     (let* ((variable (car args))
      ...
      (dynamic-parse-action-named-let-expression
       ...
       (dynamic-parse-body (extend-env-with-env
			    ...
			    nenv) body)))
     (panic "Illegal named let-expression: ~s")))
...
(define (dynamic-parse-parallel-bindings env bindings)
 ...
 (if (list-of-list-of-2s? bindings)
     ...
     (panic "Not a list of bindings: ~s")))
	 ...
	 (if (list-of-2? fst-bind)
	     (let* ((fbinding-bres
	      ...
	      (psb
	       ...
	       (cdr binds)))
	     (panic "Illegal binding: ~s"))))
       (else (panic "Illegal bindings: ~s"))))))
 ...
 (if (pair? c)
     (let ((op (car c))
      ...
      (case op
       ...
       ((begin)
	(dynamic-parse-action-begin-expression
	 (dynamic-parse-command* env args)))
...
(define (dynamic-parse-command* env commands)
 ...
 (if (list? commands)
     ...
     (panic "Invalid sequence of commands: ~s")))
...
(define (dynamic-parse-define env args)
 ...
 (if (pair? args)
	...
	(if (list-of-1? exp-or-body)
	    ...
	    (panic "Not a single expression: ~s")))
       ((pair? pattern)
	(let* ((function-name (car pattern))
	 ...
	 (dynamic-parse-action-function-definition
	  ...
	  formals-ast
	  (dynamic-parse-body
	   (extend-env-with-env env formals-env) exp-or-body))))
       (else (panic "Not a valid pattern: ~s"))))
     (panic "Not a valid definition: ~s")))
...
(define (fix tv-func)
 ...
 (let* ((new-tvar (gen-tvar))
  ...
  (if (null? inst-def)
      (panic "Illegal recursive type: ~s")
      (begin
       (set-def! new-tvar
		 (type-con inst-def)
		 (type-args inst-def))
...
(define (dynamic-parse-action-procedure-call op args)
 ;; dynamic-parse-action for procedure calls: op function, args list of
 ;; arguments
...
(define (ast-show ast)
 ...
 ;; ***Note***: check translation of constructors to numbers at the top of the
 ;; file
 (let ((syntax-op (ast-con ast))
  ...
  (case syntax-op
   ...
   ((24) (panic "Do expressions not handled! (~s)"))
   ((25) (panic "This can't happen: empty encountered!"))
   ((26) (list 'define
   ...
   ((28) (cons 'begin
	       (map ast-show syntax-arg)))
   (else (panic "Unknown abstract syntax operator: ~s")))))
...
(define (datum-show ast)
 ...
 (case (ast-con ast)
  ...
  ((7)
   (cons (datum-show (car (ast-arg ast))) (datum-show (cdr (ast-arg ast)))))
 ...
 (let ((ntvar
	(case syntax-op
	 ...
	 ((20) (let ((var-def-tvars (map ast-tvar (caadr arg)))
		...
		(for-each add-constr! var-def-tvars def-expr-types)
		(add-constr!
		 (procedure (convert-tvars var-def-tvars) body-type)
		 named-var-type)
		body-type))
	 ((23) (ast-tvar (tail arg)))
	 ((24) (panic "Do-expressions not handled! (Argument: ~s) arg"))
	 ((25) (gen-tvar))
	 ...
	 ((28) (gen-tvar))
	 (else
	  (panic "Can't handle syntax operator: ~s")))))
...
(define (convert-tvars tvar-list)
 ...
 (cond
  ...
  (else (panic "Not a list of tvars: ~s"))))
...
(define symbol-env
 (list
  ...
  (cons 'string->symbol
	(procedure (convert-tvars (list (charseq))) (symbol)))))
...
(define number-env
 (list
  ...
  (cons 'string->number
	(procedure (convert-tvars (list (charseq))) (number)))))
...
(define *forms*
 (call-with-input-file "dynamic-original.sc"
  (lambda (port)
   (define (loop forms)
    (let ((form (read port)))
     (if (eof-object? form)
