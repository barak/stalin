...
(define (canonicalize-graph graph classes)
 (define (fix node)
  ...
  (if (not (or (none-node? node) (any-node? node)))
      (begin (fix-set node green-edges set-green-edges!)
...
(define (meet graph node1 node2)
 (cond
  ...
  (else
   (let ((result
	  (make-node
	   (string-append "(" (name node1) " ^ " (name node2) ")"))))
...
(define (join graph node1 node2)
 (cond
  ...
  (else
   (let ((result
	  (make-node
	   (string-append "(" (name node1) " v " (name node2) ")"))))
    (add-graph-nodes! graph result)
...
(define (make-lattice g print?)
 ...
 (define (loop g count)
  ...
  (let ((lattice (step g)))
   (if print?
       (begin (display " -> ") (display (length (graph-nodes lattice)))))
   (let* ((new-g (reduce lattice))
	  (new-count (length (graph-nodes new-g))))
    (if (= new-count count)
	(begin (if print? (newline))
	       new-g)
	(begin
 ...
 (let ((graph
	(apply make-graph
	       (adjoin
		any-node (adjoin none-node (graph-nodes (clean-graph g)))))))
