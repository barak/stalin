...
;;; represented by a list.  The first element is some `user-defined'
;;; information associated with the token and the rest represents the token's
;;; class(es) (as a list of non-terminals that this token corresponds to).
;;;
...
;;;  V -> V y |
;;;
;;; and that the input to the parser is a string of characters.  Also, assume
;;; we would like to map the characters `x', `y', `+' and `=' into the
;;; corresponding non-terminals in the grammar.  Such a parser could be
;;; created with
;;;
;;; (make-parser
;;;   '(
...
;;;
;;; A configuration set is a configuration grouped with the set of input
;;; pointers representing where the head non-terminal of the configuration was
;;; predicted.
;;;
...
(define (make-parser grammar lexer)
 ...
 (let* ((nts (non-terminals grammar))          ; id map = list of non-terms
  ...
  (define (setup-tables grammar nts starters enders predictors steps names)
   ...
   (let ((nb-nts (vector-length nts)))
    ...
    (let nt-loop ((i (- nb-nts 1)))
     (if (>= i 0)
	 (begin (vector-set! steps i (- i nb-nts))
  ...
  ;; Now, for each non-terminal, compute the starters, enders and predictors
  ;; and the names and steps tables.
   ...
   (lambda (input)
    ...
    (define (conf-set-adjoin state conf-set conf i)
     (let ((tail (vector-ref conf-set 3))) ; put new element at tail
      ...
      (if (< tail 0)
	  (begin (vector-set! conf-set 0 (vector-ref state 0))
     ...
     (let ((state (vector-ref states state-num)))
      (let loop ((l1 l))
	   ...
	   (let* ((conf (car l1))
		...
		(begin (conf-set-adjoin state conf-set conf i)
		       (loop (cdr l1)))
		(loop (cdr l1))))))))
     ...
     (define (predict state state-num conf-set conf nt starters enders)
      ...
      (let loop1 ((l (vector-ref starters nt)))
	   ...
	   (let* ((starter (car l))
		...
		(begin (conf-set-adjoin state starter-set starter state-num)
		       (loop1 (cdr l)))
		(loop1 (cdr l))))))
	...
	(if (>= conf 0)
	    (let* ((conf-set (vector-ref state* (+ conf 1)))
	     ...
	     (let loop2 ((i head))
	      (if (>= i 0)
		  (begin (produce conf i state-num enders steps
    ...
    (define (backward states enders steps nts toks)
     (let* ((nb-toks (vector-length toks))
      ...
      (let loop2 ((i nb-toks))
       (if (>= i 0)
	   (begin (back states states* i enders steps nb-nts toks)
		  (loop2 (- i 1)))))
      states*))
