...
;;;               2-Jul-88 (Will Clinger -- distinguished #f and the empty
;;;                         list)
...
;;;    FALSEP and TRUEP use TERM-MEMBER? rather than MEMV (which is called
;;;         MEMBER in Common Lisp)
...
;;;
;;; This version of the benchmark also prints the number of rewrites as a
;;; sanity check, because it is too easy for a buggy version to return the
;;; correct boolean result.  The correct number of rewrites is
;;;
...
(define (nboyer-benchmark . args)
 (let ((n (if (null? args) 0 (car args))))
  (setup-boyer)
  (run-benchmark (string-append "nboyer" (number->string n))
		 1
		 (lambda () (test-boyer n))
		 (lambda (rewrites)
		  (and (number? rewrites)
...
(let ()
 ...
 (define (add-lemma term)
  (cond ((and (pair? term) (eq? (car term) 'equal) (pair? (cadr term)))
	 (put (car (cadr term))
	      'lemmas
	      (cons (translate-term term) (get (car (cadr term)) 'lemmas))))
	(else (panic "ADD-LEMMA did not like term:  "))))
  ...
  (let ((term (apply-subst
		...
		'((x f (plus (plus a b) (plus c (zero))))
		  ...
		  (w lessp (remainder a b) (member a (length b)))))
	       (translate-term
		(do ((term '(implies (and (implies x y)
					  (and (implies y z)
					       (and (implies z u)
						    (implies u w))))
				     (implies x w))
 ...
 (define (translate-alist alist)
  (cond ((null? alist) '())
	(else (cons (cons (caar alist) (translate-term (cdar alist)))
